    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
                ...
                ajaxloader.load("<?php echo base_url('webservices_new_dev/animals/get_user_bull_list') ?>?breed_id=&longitude=&latitude=&gender="+$("input:radio[name=inlineRadioOptions]:checked").val()+"&category_id="+$("#animal_category").val()+"&lactation=&agefrom="+$("#min_age").val()+"&users_id=&listtype="+$('#type').val()+"&page=1&ageto="+$("#max_age").val()+"&isactivated=1", function(resp){
                              var data = resp;
