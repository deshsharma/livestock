    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
                 $('.ref').show();
                ajaxloader.load("<?php echo base_url('api/get_user_type') ?>?category_id="+$("#animal_category").val()+"&latitude=30.7166839&name=&start=0&language=en&type=5&longitude=76.6946636", function(resp){
                              var data = resp;
                              var str =JSON.parse(data);
                              var tr = '';
                            if(str.error){ 
                                  ...
                                  $.each(result, function(index, item){
                                    ...
                                    if(item.image !== null){
                                      image = '<span class="card1" ><img src="'+item.image+'" alt="Avatar" width="100%"></span>';
                                        ...
                                        image = '<span class="card1" ><img src="<?= base_url('../uploads_new/profile/') ?>1615888678.jpg" alt="dealer list" style="width: 100%;"></span>';
                                    } 
                                    <?php  if($this->session->userdata("users_id") != ''){  ?>
                                      var call =  '<a onclick="Request('+item.users_id+', 0)" class="call"><i class="fa fa-phone"></i></a>';
                                    <?php }else{ ?>
                                       var call =  '<a href="<?= base_url('frontend/login') ?>" class="call"><i class="fa fa-phone"></i></a>';
                                   <?php }   ?>
                                             ...
                                             <div class="dealer_card">\
                                                <div class="row ">\
                                                   ...
                                                   <div class="col-md-8 col-8">\
                                                      <h4>'+item.contact_person+'</h4>\
                                                      <div class="address">\
                                                         <div><?= $this->webLanguage['Farm Name']?>:</div>\
                                                         <div><strong>'+item.farm_name+'</strong></div>\
                                                      </div>\
                                                      <div class="address">\
                                                         <div><?= $this->webLanguage['District']?>:</div>\
                                                         <div><strong>'+district+'</strong></div>\
                                                      </div>\
                                                      <div class="address">\
                                                         <div><?= $this->webLanguage['State']?>:</div>\
                                                         <div><strong>'+item.state+'</strong></div>\
                                                      </div>\
                                                      <div class="address">\
                                                         <div><?= $this->webLanguage['Category']?>:</div>\
                                                         <div><strong>'+item.category[0].category+'</strong></div>\
                                                      </div>\
                                                   </div>\
                                                </div>\
                                                <div class="address pt-1">\
                                                   <div><?= $this->webLanguage['ADDRESS']?>:</div>\
                                                   <div class="div">\
                                                ...
                                                <div class="location">\
                                                   ...
                                                   <div class="km"><strong>'+numb+' KM</strong> <a ><?= $this->webLanguage['See location on google']?></a></div>\
                                                </div>\
                                                <div class="see">\
                                                   <div class="see_a">\
                                                      <a href="#" class="see-animal"><?= $this->webLanguage['See animal posted by dealer']?></a>\
                                                   </div>\
                                                   <div class="see_call">'+call+'\
                                                   </div>\
                                                </div>\
                                             </div>\
                                          </div>\
