	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">COMPOSITION REQUEST MANAGEMENT</h3>
...
<script type="text/javascript">
 ...
 function status(id , status){
	if (confirm('Are you sure you want to change the status')) {
 	...
 	ajaxloader.load("<?php echo base_url('comp_request/comp_request_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person+"&name="+$('#product_name').val(), function(resp){
			                      	var data = resp;
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('comp_request/comp_request_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
                                	...
                                	delete result.count ;
			                            $.each(result, function(index, item){
                                            ...
                                            // }
											if(item.isactive == '1'){
			                            		status += "<a href='<?= base_url() ?>comp_request/view/"+item.id+"'><button type='button' class='btn btn-info btn-flat'>View</button></a>";
											}
											 tr += "<tr>\
                                                           <td>"+item.user_name+"</td>\
                                                           <td>"+item.user_mobile+"</td>\
