...
<section class="content">
	          	<div class="box-header with-border">
		              <h3 class="box-title">Supplier Management</h3>
                  <?php if($_SESSION['status'] == '6' || $_SESSION['status'] == '15'|| $_SESSION['status'] == '19'){?>
		              <a class="white" href="<?= base_url('admin/add_subuser/3'); ?>"><div class="btn btn-info but_set">
					  		Add Suppliers
		              </div></a>
                  <?php } ?>
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
		...
		if($_SESSION['status'] == 1){ ?>
			var get_url = "<?php echo base_url('api/listing_authority').'?user_type=7'?>&search="+$('#product_name').val();
		 <?php }else{ 
			if($_SESSION['type'] == '10' || $_SESSION['type'] == '11'){
								$admin_data = $this->api_model->get_seman_company_id($_SESSION['user_id']);  ?>
								var get_url = "<?php echo base_url('api/listing_authority').'?user_type=7&admin_id='.$admin_data[0]['super_admin_id'].''; ?>&search="+$('#product_name').val();
						<?php }else{ ?>
								var get_url = "<?php echo base_url('api/listing_authority').'?user_type=7&admin_id='.$_SESSION['user_id'].''; ?>&search="+$('#product_name').val();
						<?php } 
		  }
		  ?>
      					ajaxloader.load(get_url, function(resp){
                                ...
                                // delete result.count ;
			                            $.each(result, function(index, item){
                                            ...
                                            tr+="<div class='col-12 col-sm-6 col-md-4 d-flex align-items-stretch'><div class='card bg-light fix_box1'>\
