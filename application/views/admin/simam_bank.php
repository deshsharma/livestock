	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">SEMAN BANK MANAGEMENT</h3>
		              <a class="white" href="<?= base_url('admin/seman_bank_add'); ?>"><div class="btn btn-info but_set">
		                 Add Seman Bank
		              </div></a>
			                     	...
			                     	if(str['error']=='1'){ 
			                            ...
			                            $.each(result, function(index, item){
											...
											if(item.isactivated != '1'){
                                                status += 'Active';
                                                //status += "<button type='button' onclick='status("+item.selling_id+", 1)' class='btn btn-danger btn-flat'>Active</button>"
                                            }else{
                                                status += 'Deactive';
                                                //status += "<button type='button' onclick='status("+item.selling_id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
                                            }
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/seman_bank_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str['error']=='1'){ 
