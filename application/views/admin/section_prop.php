	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">SECTION PROPERTIES MANAGEMENT</h3>
		              <a class="white" href="<?= base_url('admin/section_prop_add'); ?>"><div class="btn btn-info but_set">
...
<script type="text/javascript">
 ...
 function status(id , status){
	if (confirm('Are you sure you want to change the status')) {
 	...
 	ajaxloader.load("<?php echo base_url('admin/section_prop_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person, function(resp){
			                      	var data = resp;
			                     	...
			                     	if(str.count[0].count=='0'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            status += "<a href='section_prop_edit/"+item.id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
			                            	 tr += "<tr>\
                                                           <td>"+item.name+"</td>\
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/section_prop_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
