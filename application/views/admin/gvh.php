	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">GVH MANAGEMENT</h3>
		              <a class="white" href="<?= base_url('admin/gvh_add'); ?>"><div class="btn btn-info but_set">
		                 Add GVH
		              </div></a>
...
<script type="text/javascript">
 ...
 function status(id , status){
	if (confirm('Are you sure you want to change the status')) {
 	...
 	ajaxloader.load("<?php echo base_url('admin/gvh_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person, function(resp){
			                      	var data = resp;
                                  ...
                                  delete result.count ;
			                            $.each(result, function(index, item){
											var status = '';
											if(item.is_active != '1'){
                                                status += "<button type='button' onclick='status("+item.gvh_id+", 1)' class='btn btn-danger btn-flat'>Active</button>"
																								//status += "<button type='button' onclick='status("+item.doctor_id+", 2)' class='btn btn-primary btn-flat'>Reject</button>";
                                            }else{
                                                status += "<button type='button' onclick='status("+item.gvh_id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
																								//status += "<button type='button' onclick='status("+item.doctor_id+", 2)' class='btn btn-primary btn-flat'>Reject</button>";
                                            }
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/gvh_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
