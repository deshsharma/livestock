...
class Admin_detail extends CI_Model {
	...
	public function get_role($name='', $admin_id = '', $start = '', $perpage = '')
	{
		if($perpage != ''){
	...
	public function get_video_block_by_search_title_or_subject($title='')
	{
		if($title != '' ||  $category!= ''){
			$this->db->where('title like "%'.$title.'%" OR subject like "%'.$title.'%"');
		}
		if($detail = $this->db->get('video_block')->result_array()){
			return $detail;
		}else{
	...
	public function get_video_block_any_by_users_and_group_number($users_id, $users_video_group_number, $title='', $category = '')
	{
		$this->db->where('users_video_group_number="'.$users_video_group_number.'"');
		if($title != '' ||  $category!= ''){
			$this->db->where(' title like "%'.$title.'%" AND category like "%'.$category.'%"');
		}
		if($detail = $this->db->get('video_block')->result_array()){
	...
	public function get_last_three_video_block($cat = ''){
	
		//$this->db->where()
		//$this->db->select('DISTINCT users_video_group_number, (select seris_title from video_block where users_video_group_number = vd.users_video_group_number limit 1) as seris_title, (select sublanguage from video_block where users_video_group_number = vd.users_video_group_number limit 1) as sublanguage, (select showto from video_block where users_video_group_number = vd.users_video_group_number limit 1) as showto, (select video_flag_type from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video_flag_type, (select description from video_block where users_video_group_number = vd.users_video_group_number limit 1) as description, (select number_of_videos from video_block where users_video_group_number = vd.users_video_group_number limit 1) as number_of_videos, (select subject from video_block where users_video_group_number = vd.users_video_group_number limit 1) as subject, (select category from video_block where users_video_group_number = vd.users_video_group_number limit 1) as category, (select language from video_block where users_video_group_number = vd.users_video_group_number limit 1) as language, (select start_date from video_block where users_video_group_number = vd.users_video_group_number limit 1) as start_date, (select submittedby from video_block where users_video_group_number = vd.users_video_group_number limit 1) as submittedby, (select institute from video_block where users_video_group_number = vd.users_video_group_number limit 1) as institute, (select qualifications from video_block where users_video_group_number = vd.users_video_group_number limit 1) as qualifications, (select updated_on from video_block where users_video_group_number = vd.users_video_group_number limit 1) as updated_on, (select created_on from video_block where users_video_group_number = vd.users_video_group_number limit 1) as created_on, (select user_type from video_block where users_video_group_number = vd.users_video_group_number limit 1) as user_type, (select isactivated from video_block where users_video_group_number = vd.users_video_group_number limit 1) as isactivated, (select user_id from video_block where users_video_group_number = vd.users_video_group_number limit 1) as user_id, (select link from video_block where users_video_group_number = vd.users_video_group_number limit 1) as link, (select video_id from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video_id, (select title from video_block where users_video_group_number = vd.users_video_group_number limit 1) as title, (select video from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video, (select video_thumb from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video_thumb, (select price from video_block where users_video_group_number = vd.users_video_group_number limit 1) as price');
		//$this->db->where("FIND_IN_SET(users_video_group_number, (select GROUP_CONCAT(DISTINCT users_video_group_number, ',')))");
		//$this->db->order_by('video_id', 'DESC')->limit(5);
		if($detail = $this->db->query("SELECT DISTINCT `users_video_group_number`, (select seris_title from video_block where users_video_group_number = vd.users_video_group_number limit 1) as seris_title, (select sublanguage from video_block where users_video_group_number = vd.users_video_group_number limit 1) as sublanguage, (select showto from video_block where users_video_group_number = vd.users_video_group_number limit 1) as showto, (select video_flag_type from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video_flag_type, (select description from video_block where users_video_group_number = vd.users_video_group_number limit 1) as description, (select number_of_videos from video_block where users_video_group_number = vd.users_video_group_number limit 1) as number_of_videos, (select subject from video_block where users_video_group_number = vd.users_video_group_number limit 1) as subject, (select category from video_block where users_video_group_number = vd.users_video_group_number limit 1) as category, (select language from video_block where users_video_group_number = vd.users_video_group_number limit 1) as language, (select start_date from video_block where users_video_group_number = vd.users_video_group_number limit 1) as start_date, (select submittedby from video_block where users_video_group_number = vd.users_video_group_number limit 1) as submittedby, (select institute from video_block where users_video_group_number = vd.users_video_group_number limit 1) as institute, (select qualifications from video_block where users_video_group_number = vd.users_video_group_number limit 1) as qualifications, (select updated_on from video_block where users_video_group_number = vd.users_video_group_number limit 1) as updated_on, (select created_on from video_block where users_video_group_number = vd.users_video_group_number limit 1) as created_on, (select user_type from video_block where users_video_group_number = vd.users_video_group_number limit 1) as user_type, (select isactivated from video_block where users_video_group_number = vd.users_video_group_number limit 1) as isactivated, (select user_id from video_block where users_video_group_number = vd.users_video_group_number limit 1) as user_id, (select link from video_block where users_video_group_number = vd.users_video_group_number limit 1) as link, (select video_id from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video_id, (select title from video_block where users_video_group_number = vd.users_video_group_number limit 1) as title, (select video from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video, (select video_thumb from video_block where users_video_group_number = vd.users_video_group_number limit 1) as video_thumb, (select price from video_block where users_video_group_number = vd.users_video_group_number limit 1) as price FROM `video_block` as `vd` WHERE FIND_IN_SET(users_video_group_number, (select GROUP_CONCAT(DISTINCT users_video_group_number, ','))) ORDER BY `video_id` DESC LIMIT 5")->result_array()){
		//if($detail = $this->db->get('video_block as vd')->result_array()){
			return $detail;
		}else{
			return false;
		}
