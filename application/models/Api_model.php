...
class Api_model extends CI_Model {
    ...
    public function get_crone_data(){
        date_default_timezone_set('Asia/Calcutta');
        $my_date_time_from = date("Y-m-d h:i:s", strtotime("-".TREATMENT_CANCEL_TIME." hours"));
        $my_date_time_to = date("Y-m-d h:i:s", strtotime("-".TREATMENT_CANCEL_TIME_TO." hours"));
        // echo "select * from vt_requests where created_on BETWEEN '".$my_date_time_to."' AND '".$my_date_time_from."' AND status ='0'";
        // exit;
        return $this->db->query("select * from vt_requests where created_on BETWEEN '".$my_date_time_to."' AND '".$my_date_time_from."' AND status ='0'")->result_array();
    }
    public function get_dewarming(){
   ...
   public function get_user_wallets($users_id,$type,$payment_type){
    ...
    public function get_user_auth_tocken($users_number, $type){
        $dateNow = date('Y-m-d H:i:s');
    ...
    public function get_user_auth_users($users_auth_tocken){
        ...
        if($details) {
            ...
            if($details[0]['user_type'] == '1') {
                $data = $this->get_fcm_user($splitValues[0]);
                $data[0]['user_type'] = 1;
                return $data;
            } else if($details[0]['user_type'] == '0'){
                $data =  $this->get_doc_detail_id($splitValues[0]);
                $data[0]['user_type'] = 0;
                return $data;
            }else{
                $data = $this->get_fcm_user($splitValues[0]);
    ...
    public function remove_whishlist_list($id){
        $detail = $this->db->where('id = "'.$id.'"')->delete('video_interest');
        return True;
    }
}
