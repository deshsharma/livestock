...
class New_api extends CI_Controller {
	...
	public function dealer_accept(){
		...
		if($id == ''){
			...
			$product_name = $this->api_model->get_data('id = "'.$orders[0]['product_id'].'"', 'product');
			//  print_r($orders[0]['product_id']);
			// exit;
			if(!empty($orders)){
				if($this->api_model->update('id', $id, 'product_order', $data)){
					...
					$json['msg'] = "Your order has been successfully Accepted";
					 		...
					 		$title = 'Order Status';
                            $msg1 = "Your Order of '".$product_name[0]['name']."' has been accepted.";
                            $msg['users_id'] = $orders[0]['users_id'];
                            $msg['title'] = $title;
                            $msg['message'] = $msg1;
                            ...
                            $old_msg['to_users_id'] = $orders[0]['users_id'];
                            $old_msg['to_id'] = $orders[0]['users_id'];
                            ...
                            $this->simple_push_none($orders[0]['users_id'], 2 , $title, '1', $msg1);
				}else{
					$json['success']  = false; 
	...
	public function get_delivery_report_delivery_partner(){
		...
		if($insentive != ''){
			...
			$date = "AND order_type = '0' AND order_payment_status = '4' AND isactive = '5' ";
		...
		if($id == ''){
			...
			$json['error'] = 'Please Send User ID';
				...
				if($admin_data[0]['type'] != '18'){
					if($order = $this->api_model->query_build('SELECT `id`, `product_qty`, `date` as `order_date`, order_type, CONCAT("https://www.livestoc.com/api/product_invoice/", id) as invoice, (select fullname from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_name, (select address1 from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_address1, (select latitude from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_latitude, (select longitude from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_longitude, (select address2 from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_address2, (select city from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_city, (select district from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_district, (select postal_code from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_postal_code, (select address_type from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_address_type, (select mobile from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_mobile, `isactive` as `order_status`, (select name from product where id = product_id) as product_name, (select brand from product where id = product_id) as product_brand, (select images from product where id = product_id) as product_image, (select sku from product where id = product_id) as product_sku, (select shor_desc from product where id = product_id) as product_shor_desc, (select long_desc from product where id = product_id) as product_long_desc, (select other_desc from product where id = product_id) as product_other_desc, (select discount from product where id = product_id) as product_discount, `product_qty`, `package_price`, (select name from product_package where id = package_id) as package_name FROM `product_order` as `pro` WHERE FIND_IN_SET('.$id.', distributor_id) AND (select id from product_order_rejection_log where order_id = pro.id and vendor_id = '.$id.') IS NULL '.$where.' order by id DESC '.$limit.'')){
					...
					if($order = $this->api_model->query_build('select id, product_qty, date as order_date,  CONCAT("https://www.livestoc.com/api/product_invoice/", id) as invoice, order_type, (select fullname from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_name, (select address1 from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_address1,  (select address2 from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_address2, (select latitude from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_latitude, (select longitude from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_longitude, (select city from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_city, (select district from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_district, (select images from product where id = product_id) as product_image, (select postal_code from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_postal_code, (select address_type from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_address_type, (select mobile from address_mst where address_id = (select address_id from users where users_id = pro.users_id)) as user_mobile, isactive as order_status, (select name from product where id = product_id) as product_name, (select brand from product where id = product_id) as product_brand, (select sku from product where id = product_id) as product_sku, (select shor_desc from product where id = product_id) as product_shor_desc, (select long_desc from product where id = product_id) as product_long_desc, (select other_desc from product where id = product_id) as product_other_desc, (select discount from product where id = product_id) as product_discount, product_qty, package_price, (select name from product_package where id = package_id) as package_name from product_order as pro where FIND_IN_SET(pro.product_id ,(select GROUP_CONCAT(pd.id) from product as pd where pd.user = '.$id.')) AND (select id from product_order_rejection_log where order_id = pro.id and vendor_id = '.$id.') IS NULL '.$where.' '.$limit.'')){
	...
	// 				if(!empty($ot)){
	// 					if($this->api_model->update('id', $id, 'product_order', $data)){
	// 						$user_details = $this->api_model->get_data('id = "'.$id.'" AND isactive = "'.$data['isactive'].'"', 'product_order');
	// 						if($user_details[0]['order_type'] == '0'){
	// 							if($product_offer = $this->api_model->get_data('product_pack = "'.$user_details[0]['package_price_id'].'" AND product_id = "'.$user_details[0]['product_id'].'"', 'product_offer')){
	// 								$wall['log_id'] = $user_details[0]['log_id'];
	// 								$wall['users_id'] = $user_details[0]['users_id'];
	// 								$wall['type'] = '14';
	// 								$wall['animal_id'] = $user_details[0]['id'];
	...
	// 								$title = 'Order Status';
	// 								$msg1 = "Your Order has been delivered successfully.";
	// 								$msg['users_id'] = $user_details[0]['users_id'];
	// 								$msg['title'] = $title;
	// 								$msg['message'] = $msg1;
	// 								$msg['date'] = date('Y-m-d h:i:s');
	// 								$msg['type'] = '4';
	...
	// 								$msg['flag'] = '4';
	// 								$this->api_model->user_notification($msg);
	// 								$old_msg['to_users_id'] = $user_details[0]['users_id'];
	// 								$old_msg['to_id'] = $user_details[0]['users_id'];
	// 								$old_msg['to_type'] = 'Dealer';
	// 								$old_msg['title'] = $title;
	// 								$old_msg['from_type'] = 'Livestoc Team';
	// 								$old_msg['success'] = '1';
	...
	// 								$old_msg['date_added'] = date('Y-m-d h:i:s');
	// 								$this->api_model->old_notification($old_msg);
	// 								$this->simple_push_none($user_details[0]['users_id'], 2 , $title, '1', $msg1);
	// 					}else{
	// 						$json['success']  = false; 
	// 						$json['error'] = "Database error";
	// 					}
	...
	// 				if($data['isactive'] == '3'){
	// 					$title = 'Order Status';
	// 					$msg1 = "Your Order status has been changed to In Transit.";
	// 					$msg['users_id'] = $user_details[0]['users_id'];
	// 					$msg['title'] = $title;
	// 					$msg['message'] = $msg1;
	// 					$msg['date'] = date('Y-m-d h:i:s');
 ...
 //                    $this->simple_push_none($user_details[0]['users_id'], 2 , $title, '1', $msg1);
		...
		if($id == ''){
			...
			if($status == '5'){
				if($otp != ''){
					$ot = $this->api_model->get_data('id = '.$id.' AND otp = "'.$otp.'"', 'product_order');
					if(!empty($ot)){
						$data['distance_covered'] = $distance_covered;
	...
	public function get_ai_detail($animal_id){
		if($data = $this->api_model->get_data('animal_id = "'.$animal_id.'" AND treat_type = "3" AND status = "4"', 'vt_requests', 'id DESC', 'id, concat("'.base_url().'api/vetinvoice/", Invoice_id) as Invoice_id, vt_id, vacc_id, date')){
			...
			foreach($data as $da){
				...
				$bull_data = $this->api_model->get_data('id ="'.$da['vacc_id'].'"', 'bull_table','','id, concat("LIVE_", id) as bull_no, bull_id, (select br.breed_name from breed as br where br.breed_id = bread) as bread_name, concat("'.base_url().'uploads/bank/",image) as image, (Select sg.group from semen_group as sg where sg.id = groups) as groups_name');
				$detail[$i] = $da;
	...
	public function get_animal_yield($animal_id, $date_from = '', $data_to = ''){
		$where = '';
		if($animal_id != ''){
	...
	// 		$cate[$i]['price'] = $this->api_model->get_data('FIND_IN_SET(id, "'.$cat['unit'].'")', 'unit', '', 'id, name');;
	...
	public function make_invoice(){
		...
		if($request_id != ''){
			...
			$semen_group1 = $this->api_model->query_build('SELECT min(id) as id, farmer_price, farmer_offer_price, ai_price, ai_farmer_price FROM seman_stock where admin_id = "'.$data['admin_id'].'" AND bull_id = "'.$data['bull_id'].'" AND rest_stock <> 0 AND is_update = "1"');
			...
			if($request[0]['order_type'] == '1'){
				...
				if($semen_group1[0]['ai_farmer_price'] != '' && $semen_group1[0]['ai_farmer_price'] != '0'){
			...
			if($request[0]['vacc_id'] == '' || $request[0]['vacc_id'] == '0'){
				...
				if($request[0]['vacc_id'] != $data['bull_id']){
					$old_bull2 = $this->api_model->get_data('id = "'.$data['bull_id'].'"' , 'bull_table', '', '*');
					$old_semen_group2= $this->api_model->get_data('id ="'.$old_bull2[0]['groups'].'"', 'semen_group','','*');
					...
					$old_semen_group1 = $this->api_model->query_build('SELECT min(id) as id, farmer_price, farmer_offer_price, ai_price, ai_farmer_price FROM seman_stock where admin_id = "'.$data['admin_id'].'" AND bull_id = "'.$request[0]['vacc_id'].'" AND rest_stock <> 0 AND is_update = "1"');
					$old_semen_group3 = $this->api_model->query_build('SELECT min(id) as id, farmer_price, farmer_offer_price, ai_price, ai_farmer_price FROM seman_stock where admin_id = "'.$data['admin_id'].'" AND bull_id = "'.$data['bull_id'].'" AND rest_stock <> 0 AND is_update = "1"');

					if($request[0]['order_type'] == '1'){
						$pack_data = $this->api_model->get_data('users_id = "'.$users_id.'"', 'ai_package_log', '', 'sum(rest_quantity) as sum');
						if($pack_data[0]['sum'] >  0){
							$per = $old_semen_group2[0]['farmer_offer_price'];
							$old_per = $old_semen_group[0]['farmer_offer_price'];
						}else{
							$per = $old_semen_group2[0]['farmer_price'];
							$old_per = $old_semen_group[0]['farmer_price'];
						}
					}else{
						if($old_semen_group1[0]['ai_farmer_price'] != '' && $old_semen_group1[0]['ai_farmer_price'] != '0'){
							$per = $old_semen_group1[0]['ai_farmer_price'];
							$old_per = $old_semen_group3[0]['ai_farmer_price'];
						}else{
							$per = $old_semen_group[0]['farmer_price'];
							$old_per = $old_semen_group2[0]['farmer_price'];
						}
					}
		...
		if($id = $this->api_model->submit('semen_invoice_performa',$data)){
			$msg = "Please Pay Rs = ".$data['invoice_total']."to the Service Provider and OTP is ".$data['otp']."  https://www.livestoc.com/harpahu_merge/api/invoice/".$id;
	...
	public function complite_invoice(){	
		...
		if($data = $this->api_model->check_request_otp($in[0]['request_id'], $otp)){
			...
			if($data[0]['log_id'] == '0'){
				...
				$bull2 = $this->api_model->get_data('id = "'.$in[0]['bull_id'].'"' , 'bull_table', '', '*');
				$semen_price2 = $this->api_model->get_data('id ="'.$bull2[0]['groups'].'"','semen_group','','*');
				...
				$old_bull1 = $this->api_model->get_data('id = "'.$in[0]['old_bull_id'].'"' , 'bull_table', '', '*');
				$semen_old_price1 = $this->api_model->get_data('id ="'.$old_bull1[0]['groups'].'"','semen_group','','*');
				if($order_type == '1'){
					$pack_data = $this->api_model->get_data('users_id = "'.$data[0]['users_id'].'"', 'ai_package_log', '', 'sum(rest_quantity) as sum');
					if($pack_data[0]['sum'] >  0){
						$per = $semen_price2[0]['farmer_offer_price'];
						$old_per = $semen_old_price1[0]['farmer_offer_price'];
					}else{
						$per = $semen_price2[0]['farmer_price'];
						$old_per = $semen_old_price1[0]['farmer_price'];
					}
				}else{
					if($semen_price[0]['ai_farmer_price'] != '' && $semen_price[0]['ai_farmer_price'] !='0'){
						$per = $semen_group1[0]['ai_farmer_price'];
						$old_per = $semen_old_price[0]['ai_farmer_price'];
					}else{
						$per = $semen_price2[0]['farmer_price'];
    ...
    public function get_company_bull(){
		...
		if($data = $this->api_model->get_company_bull($breed_id, $category_id, $limit, $premium, $daughter_yield_from, $daughter_yield_to, $user_type, $price_from, $price_to, $milk_type, $price_order, $latitude, $longitude,  $start, $bull_id)){
			$dommy = [];
					if(isset($breed_id)){
						if($primium != '1'){
                ...
                $json['data'] = $dommy;
                //$json['count'] = $i;
				$json['count'] = $this->api_model->get_company_bull_count($breed_id, $category_id, $limit, $premium, $daughter_yield_from, $daughter_yield_to, $user_type, $price_from, $price_to, $milk_type, $price_order, $latitude, $longitude,  $start);
			}else{
				$json['success']  = false; 
				if($start <= 1){
	...
	public function get_animal_yield_by_doctor(){
		...
		if($data = $this->api_model->query_build('SELECT DISTINCT created_date, animal_id, (select avg(ay.yield) from animals_yield_check as ay where ay.created_date = ayc.created_date AND ay.animal_id = ayc.animal_id ) as yeild_avg from animals_yield_check as ayc '.$where)){
			...
			foreach($data as $da){
				$detail[$i] = $da;
				$der = $this->api_model->query_build('SELECT id, users_id, CONCAT("http://amazebrandlance.com/uploads/yield/",video) as video, yield, fat_snf, lactation_period ,last_calving_date,created_time from animals_yield_check where created_date = "'.$da['created_date'].'" AND animal_id = "'.$da['animal_id'].'" ORDER BY created_time DESC');
				$detail[$i]['records'] = $der;
				$i++;
			}
			$json['success']  = true; 
	...
	public function get_json_dashboard(){
		...
		$data = $this->api_model->get_data('section_id = "'.$section_id.'"','dashboard_crone','','value');
		if(!empty($data)){
			$json  = $data[0]['value']; 
		}else{
			$json['success']  = false; 
	...
	public function get_vehicals_details(){
		$data = $this->api_model->get_data('is_activated = "1"','vehicals_type','','*,CONCAT("https://www.livestoc.com/harpahu_merge/uploads/category/",image) as image');
		if($data){
			$json['success']  = true; 
			$json['data']  = $data; 
		}else{
	...
	public function get_near_by_ai_worker(){
		...
		// $count = $this->api_model->query_build("select count(do.doctor_id) as count,do.refral_code,do.city,do.state,do.doctor_id, do.username,do.email, do.total_experience, do.image, if((select GROUP_CONCAT(id) as id from vt_requests where vt_id = do.doctor_id AND treat_type = '3' AND status = '4') IS NOT NULL, 1,0) as status, (IFNULL(( 3959 * acos( cos( radians('".$lat."') ) * cos( radians( cl.lantitute ) ) * cos( radians( cl.langitute ) - radians ('".$lang."') ) + sin( radians('".$lat."') ) * sin( radians( cl.lantitute ) ) ) ),0)) AS distance from current_loc as cl, doctor as do where do.doctor_id = cl.doctor_id AND do.users_type IN ('pvt_ai', 'pvt_vt')  AND do.isactivated = '1' ".$where."  ORDER BY ".$order_by." LIMIT ".$start.", ".$perpage."");
		// print_r($data);
		// exit;
		if(!empty($data)){
		// exit;
		// if($data = $this->api_model->get_ai_doc_stoc('pvt_ai, pvt_vt', $lang, $lat, $bull_id)){
				foreach($data as $d){
					...
					$d['qualification'] = $degree == false ? [] : $degree;
					$d['image'] = base_url().'harpahu_merge/uploads/doc/'.$d['image'];
					$d['no_of_ai'] = $no_of_ai_done[0]['count'];
					$d['succes_ai'] = '';
					$d['succes_rate'] = '';
					$d['rating'] = '';
