...
class Animal extends CI_Controller {
	...
	public function tert_req_ac_re(){
	    ...
	    if(!isset($users_id) || !$users_id){
			...
			$data['success'] =  TRUE;
			$data['msg'] =  "Your request has been ".$req."";
				$msg['message'] = "Your request has been ".$req."";
				$msg['users_id'] = $users_id;
				$msg['type'] = 1;
				$msg['title'] = "Treatment";
				$msg['date'] = date('Y-m-d h:i:s'); 
				...
				$this->push_non($th->parent_id, 1 , $msg['title'], $msg['message'], $msg['title']);
		}
		echo json_encode($data);
	}
	public function request_detail(){
		...
		if(!isset($request_id) || !$request_id){
			...
			$ani_d = [];
			$r =[];
			$detail = $this->api_model->request_track_detail($request_id);
			$user_data = $this->api_model->get_user_info_id($detail[0]['user_id']);
			...
			if($request_det[0]['treat_type'] == 3){
				...
				$seman_data = $this->api_model->get_seman_detail($request_det[0]['treat_type']);
				$user_data[0]['seman_price'] = $seman_data[0]['price'];
				$breed_name = $this->api_model->get_animal_breed($seman_data[0]['bread']);
			}else{
				$user_data[0]['seman_price'] = '';
			}
			$user_data[0]['date'] = $request_det[0]['date'];
			$user_data[0]['image'] = base_url().'uploads/image/profile.jpg';
			foreach($detail as $d){
				//print_r($d);
