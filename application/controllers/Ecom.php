...
class Ecom extends CI_Controller {
    ...
    public function get_product_type(){
        ...
        if($category != ''){
           $category = $this->product_cat_bind($category);
            $where .= " FIND_IN_SET(category, '".$category."') AND";
        }
        if($section != ''){
            $where .= ' FIND_IN_SET("'.$section.'",`section`) AND';
        }
    ...
    public function cash_on_delivery(){
           ...
           // $row = $this->api_model->get_data('id = "'.$purchase_id.'"', 'log_file');
            ...
            foreach($product_id as $pur){
             ...
             //$to = 'shahiakhilesh75@gmail.com';
                    // $message = 'You have a new Order request for "Product Name" by "User Name" from "Location of user"';
                    // $email = "this is test";
                    // $curl = curl_init();
                    // curl_setopt_array($curl, array(
                    //     CURLOPT_URL => "https://amazebrandlance.com/test/send_test_mail/?message=".$message."",
