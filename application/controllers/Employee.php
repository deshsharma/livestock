...
class Employee extends CI_Controller {
    ...
    public function edit($id){
        $admin_data['data'] = $this->api_model->get_data('admin_id = "'.$id.'"', 'admin');
    ...
    public function employee_user_search(){
		...
		$start = $this->input->get_post('start');
		$perpage = 10;
        $where = 'admin_id = "'.$id.'" OR users_id = (select users_id from ai_package_log where admin_id = "'.$id.'")';
        if($name != ''){
            $where = ' AND fname like "%'.$name.'% "';
        }
