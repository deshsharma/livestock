...
class Vetreg extends CI_Controller {
    ...
    public function homepage(){
        $this->load->view('vet_reg/homepage.php');
    }
    ...
    public function reg2(){
        $d['mobile'] = $_REQUEST['mobile'];
        //$this->session->set_userdata('reg2', $d);
        // exit;
        // print_r($this->session->userdata("reg2", $data));
        if($_POST['submit']){
            //print_r($_REQUEST);
            ...
            if($this->form_validation->run('add_bank')){
                ...
                if($data['passcode'] != $confirm_pass){
                    ...
                    $this->load->view('vet_reg/registration/header');
                    $this->load->view('vet_reg/registration/reg2', $data);
                }else if($this->api_model->docmobileadhaarcheck($data['mobile'])){
                    ...
                    if($_FILES){
                        ...
                        if($_FILES['aadharcar_image']['name']){
                            $_FILES['file']['name']       = $_FILES['aadharcar_image']['name'];
                            $_FILES['file']['type']       = $_FILES['aadharcar_image']['type'];
                            $_FILES['file']['tmp_name']   = $_FILES['aadharcar_image']['tmp_name'];
                            $_FILES['file']['error']      = $_FILES['aadharcar_image']['error'];
                            $_FILES['file']['size']       = $_FILES['aadharcar_image']['size']; 
                            $config = array();
                            $config['upload_path'] = '/var/www/html/harpahu_merge_dev/uploads/reg_images/';
                            ...
                            $data['addarcardimage'] = $image_name;
                        }
                    ...
                    return redirect(base_url().'vetreg/reg3_edu');
                }
            }else{
                $this->load->view('vet_reg/registration/header');
                $this->load->view('vet_reg/registration/reg2');
            }
        } else {
            $this->load->view('vet_reg/registration/header');
            $this->load->view('vet_reg/registration/reg2');
                            ...
                            if($this->form_validation->run('add_bank')){
                                if($_FILES){
                                    ...
                                    if($_FILES['documents']['name']){
                                        $_FILES['file']['name']       = $_FILES['documents']['name'];
                                        $_FILES['file']['type']       = $_FILES['documents']['type'];
                                        $_FILES['file']['tmp_name']   = $_FILES['documents']['tmp_name'];
                                        $_FILES['file']['error']      = $_FILES['documents']['error'];
                                        $_FILES['file']['size']       = $_FILES['documents']['size']; 
                                        $config = array();
                                        $config['upload_path'] = '/var/www/html/harpahu_merge_dev/uploads/reg_images/';
                                        $config['allowed_types'] = 'gif|jpg|png|jpeg';
                                        $config['max_size']      = '5000';
                                    ...
                                    if($_FILES['educaton_image1']['name']){
                                        $_FILES['file']['name']       = $_FILES['educaton_image1']['name'];
                                        $_FILES['file']['type']       = $_FILES['educaton_image1']['type'];
                                        $_FILES['file']['tmp_name']   = $_FILES['educaton_image1']['tmp_name'];
                                        $_FILES['file']['error']      = $_FILES['educaton_image1']['error'];
                                        $_FILES['file']['size']       = $_FILES['educaton_image1']['size']; 
                                        $config = array();
                                        $config['upload_path'] = '/var/www/html/harpahu_merge_dev/uploads/reg_images/';
                                        $config['allowed_types'] = 'gif|jpg|png|jpeg';
                                        $config['max_size']      = '5000';
                                    ...
                                    if($_FILES['educaton_image2']['name']){
                                        $_FILES['file']['name']       = $_FILES['educaton_image2']['name'];
                                        $_FILES['file']['type']       = $_FILES['educaton_image2']['type'];
                                        $_FILES['file']['tmp_name']   = $_FILES['educaton_image2']['tmp_name'];
                                        $_FILES['file']['error']      = $_FILES['educaton_image2']['error'];
                                        $_FILES['file']['size']       = $_FILES['educaton_image2']['size']; 
                                        $config = array();
                                        $config['upload_path'] = '/var/www/html/harpahu_merge_dev/uploads/reg_images/';
                                        $config['allowed_types'] = 'gif|jpg|png|jpeg';
                                        $config['max_size']      = '5000';
                                    ...
                                    if($_FILES['educaton_image3']['name']){
                                        $_FILES['file']['name']       = $_FILES['educaton_image3']['name'];
                                        $_FILES['file']['type']       = $_FILES['educaton_image3']['type'];
                                        $_FILES['file']['tmp_name']   = $_FILES['educaton_image3']['tmp_name'];
                                        $_FILES['file']['error']      = $_FILES['educaton_image3']['error'];
                                        $_FILES['file']['size']       = $_FILES['educaton_image3']['size']; 
                                        $config = array();
                                        $config['upload_path'] = '/var/www/html/harpahu_merge_dev/uploads/reg_images/';
                                        $config['allowed_types'] = 'gif|jpg|png|jpeg';
                                        $config['max_size']      = '5000';
                                ...
                                return redirect(base_url().'vetreg/reg4_experience');
                            }else{
                                 $this->load->view('vet_reg/registration/header');
                                $data['years'] = $years;
                                $data['specialization'] = $specializationList;
                                $this->load->view('vet_reg/registration/reg3_edu', $data);
                            } 
                    }else{
                        $this->session->set_userdata('reg3_edu', $data);
                        return redirect(base_url().'vetreg/reg4_experience');
    ...
    public function reg4_experience(){
        ...
        //if(!empty($this->session->userdata("reg2")) && !empty($this->session->userdata("reg3_edu"))) {
            if(isset($_POST['submit'])){
				...
				$expe_list = $this->input->get_post('specialisation_list');
				$data['exp_list'] = implode(",",$expe_list);
				$data['descriptions'] = $this->input->get_post('descriptions');
                //$data['experince_list'] = explode(',',$this->input->get_post('specialisation_list'));
               ...
               // print_r($designation);
                ...
                foreach($name_organization as $na){
                 ...
                 if($this->input->get_post('telephonicConsult') != 'on'){
                        ...
                        if (!empty($this->input->get_post('specialisation_list'))){
                            $this->form_validation->set_rules('specialisation_list',"Expertise", "trim|xss_clean|numeric");
                        } else {
                            $this->form_validation->set_rules('specialisation_list',"Expertise", "trim|required");
                        }
                        ...
                        if($this->form_validation->run('add_bank')){
                            ...
                            return redirect(base_url().'vetreg/reg5_regdetails');
                        }else{
                            $this->load->view('vet_reg/registration/header');
                            $this->load->view('vet_reg/registration/reg4_experience', $data);
                        }
                }else{
                        $this->session->set_userdata('reg4_experience', $data);
                        return redirect(base_url().'vetreg/reg5_regdetails');
                 ...
                 // return redirect(base_url().'vetreg/reg5_regdetails');
                $this->load->view('vet_reg/registration/header');
                $this->load->view('vet_reg/registration/reg4_experience');
            }
        ...
        //if(!isset($_POST['telephonicConsult'])){
            ...
            //     && !empty($this->session->userdata("reg4_experience"))) {
                ...
                if($_POST['submit']){
                    ...
                    $data['state_year_of_reg'] = $this->input->get_post('state_year_of_reg');
                    $data['state_veterinary_council'] = $this->input->get_post('state_veterinary_council');
                    $data['state_reg_number'] = $this->input->get_post('state_reg_number');
					$data['vc_certificate'] = $_REQUEST['ani_certificate'];
					$data['state_vci_certificate'] = $_REQUEST['animal_image'];
                        $this->form_validation->set_rules('state_year_of_reg','Please Enter Regisration number','numeric|required|trim');
                        $this->form_validation->set_rules('state_veterinary_council','Please Enter Council','required|trim');
                        $this->form_validation->set_rules('state_reg_number','Please Enter Year Regisration','required|trim');
                        $this->form_validation->set_error_delimiters('<div class="error">', '</div>');              
                        if($this->form_validation->run('add_bank')){
    ...
    public function reg6_bankdetails(){
        ...
        //     && !empty($this->session->userdata("reg3_edu")) 
        //     && !empty($this->session->userdata("reg4_experience"))  
        //     && !empty($this->session->userdata("reg5_regdetails"))) {
                if($_POST['submit']){
                    if(!isset($_POST['telephonicConsult'])){
                        ...
                        if($this->form_validation->run('add_bank')){
                            ...
                            return redirect(base_url().'vetreg/reg7_language');
                        }else{
                            $this->session->set_userdata('reg6_bankdetails', $data);
                            return redirect(base_url().'vetreg/reg7_language');
                        }
                        ...
                        $this->load->view('vet_reg/registration/header');
                        $this->load->view('vet_reg/registration/reg6_bankdetails', $data);
                }
        // } else {
        //     return redirect(base_url().'vetreg/reg2');
        // }
    ...
    public function reg7_language(){
        ...
        //     && !empty($this->session->userdata("reg4_experience"))  
        //     && !empty($this->session->userdata("reg5_regdetails")) 
        //     && !empty($this->session->userdata("reg6_bankdetails"))) {
            ...
            if($_POST['submit']){
                ...
                if($this->form_validation->run('add_bank')){
                   ...
                   /* print_r('reg5_regdetails.....');
                    ...
                    print_r('reg7_language.....');
                    print_r($this->session->userdata("reg7_language"));*/
                   //die();
                    // print_r($_SESSION['reg2']);
                    // print_r('reg2');
                    // print_r($_SESSION['reg3_edu']);
                    // print_r('reg3_edu');
                    // print_r($_SESSION['reg4_experience']);
                    // print_r('reg4_experience');
                    // print_r($_SESSION['reg5_regdetails']);
                    // print_r('reg5_regdetails');
                    // print_r($_SESSION['reg6_bankdetails']);
                    // print_r('reg6_bankdetails');
                    // print_r($_SESSION['reg7_language']);
                    // print_r('reg7_language');
                    


                    $add_attributes = array_merge(
                        $this->session->userdata("reg2"),
                        $this->session->userdata("reg3_edu"),
                        $this->session->userdata("reg4_experience"),
                        $this->session->userdata("reg5_regdetails"),
                        $this->session->userdata("reg6_bankdetails"),
                        $this->session->userdata("reg7_language")
                    );
                    //print_r($add_attributes);
                    // exit;
                    $reg3_edu = $this->session->userdata("reg3_edu");
                    $reg4_experience = $this->session->userdata("reg4_experience");
                    $data['adhaar_img'] = $add_attributes['addarcardimage'];
                    $data['username'] = $_SESSION['reg2']['first_name'];
                    $data['gender'] = $_SESSION['reg2']['gender'];
                    $data['email'] = $_SESSION['reg2']['email'];
                    $data['mobile_code'] = $add_attributes['mobile_code'];
					...
					$data['state_year_of_reg'] = $_SESSION['reg5_regdetails']['state_year_of_reg'];
					$data['state_veterinary_council'] = $_SESSION['reg5_regdetails']['state_veterinary_council'];
					$data['state_reg_number'] = $_SESSION['reg5_regdetails']['state_reg_number'];
					$data['vc_certificate'] = $_SESSION['reg5_regdetails']['vc_certificate'];
					$data['state_vci_certificate'] = $_SESSION['reg5_regdetails']['state_vci_certificate'];
                    ...
                    //$data['state_code'] = $_SESSION['reg2']['pincode'];
                    //$data['district'] = $_SESSION['reg2']['place'];
                    $data['district_code'] = '';
                    //$data['consul_fee'] = $consul_fee;
                    //$data['expertise_list'] =  $_SESSION['reg4_experience']['experince_list'];
                    $data1['descriptions'] = $_SESSION['reg4_experience']['descriptions'];
					$data['expertise_list'] = $_SESSION['reg4_experience']['exp_list'];
                   /* echo "<pre>";
                    print_r($data['expertise_list']);

                    if(!empty($data['expertise_list'])){
                            foreach ($data['expertise_list'] as $key => $value) {
                                $exp_data['doc_id'] = $last_id;
                                $exp_data['designation'] = $data['expertise_list'][$key]['designation'];
                                $exp_data['from_date']=  $data['expertise_list'][$key]['fromdate'];
                                $exp_data['organization']= $data['expertise_list'][$key]['name_organization0'];
                                $exp_data['to_date']=  $data['expertise_list'][$key]['fromtodate'];
                                $exp_data['year']= '';
                ...
                //print_r($data);
                    if($last_id = $this->api_model->insert_doc_info($data)){
                        ...
                        $last_data['image'] = '/var/www/html/harpahu_merge_dev/uploads/reg_images/'.$last_data['image'];;
                        //base_url().'uploads/doctor/'.$last_data['image'];
                        $json['success'] = TRUE;
                        $json['msg'] =  "Its Done";
                        $json['data'][] = $last_data;
                        // die('dasdasd');
