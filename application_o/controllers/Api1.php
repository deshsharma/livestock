...
class Api extends CI_Controller {
	...
	public function test_push(){
		...
		$msg = 'test';
		$flag = '1';
		$server_key = 'AIzaSyBLMIkhQ3Rt5Q5a-KO2ApXiKUpxPIuLtA8';
		$fcm = 'ekq1mgbYadA:APA91bE0SBqFg2mV3moWwWBw9ID9JQ1bgoT8GvqYKzJ--6vHyc5tXqoKJXse_c8Xhiu4H16VbgDT5utQROPuVLR5W8WgRxmWBMcxsqHC2Dlp6tURx3KvTgMn2Uhbar_76iF2eyK5qF02';
	...
	public function test_call_api(){
		...
		$d = $this->api_model->get_semen_price($type, $bull_id);
			//foreach($d as $ds){
				...
				$data[6]['price'] = $d[0]['price'] + $d[0]['vt_ai_price'] + $data[3]['semen_price'];
			//}
			// /print_r($d);
			// exit;
	...
	public function lead_dealer_breader(){
		...
		if(!$this->api_model->get_data('users_id = "'.$data['users_id'].'" AND status = "0" AND lead_user_id = "'.$data['lead_user_id'].'"' , 'lead_breader_dealer', '', '*')){
	...
	public function get_call_history(){
		...
		//print_r($number1);
		$detail= $this->api_model->get_doc_call_history($number1);
		if(empty($detail)){
			$json['success'] = false;
			$json['error'] = 'No record found.';
	...
	public function call_inisite(){
		...
		curl_setopt_array($curl, array(
		  ...
		  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		  CURLOPT_CUSTOMREQUEST => "POST",
		  CURLOPT_POSTFIELDS => "{\n  \"k_number\": \"+916366783212\",\n  \"agent_number\": \"$number1\",\n  \"customer_number\": \"$number2\",\n  \"caller_id\": \"+918047248869\"\n}",
		  CURLOPT_HTTPHEADER => array(
		    "authorization: 95d4c58b-80a1-4e75-bf78-5970a52d0c11",
		    "cache-control: no-cache",
		    "postman-token: 0525face-1e1b-29f2-3216-f1b533a0b4f3",
		    //"timer: 00:00:60",
		    "timer: $time",
		    "x-api-key: nDRUngYws3739teklCMYp9IufbQ8qjf212iyMkiG"
		  ),
   ...
   //      	print_r($animal);
	...
	public function get_all_doc_by_visit(){
		...
		$state = $this->input->get_post('state_code');
		$visit = 1;
			$detail = $this->api_model->get_doc_quif_speci_list($speci_type, $premium_type, $latitu, $langitude, $languages, $name, $expertise, $visiting_set, $start, $visit);
			$data = [];
			$i = 0;
			foreach($detail as $d){	
				//if($d['doc_id'] != ''){
	...
	public function register_doc(){
		...
		$online_for_visit = $this->input->get_post('online_for_visit');
		$avaialable_for_visit = $this->input->get_post('avaialable_for_visit');
		//$is_consultation_on = $this->input->get_post('is_consultation_on');
	...
	public function get_coustomer_semen_order(){
		$users_id = $this->input->get_post('users_id');
		$data = $this->api_model->get_coustomer_semen_order1($users_id);
		//print_r($data);
		$detail = '';
		foreach ($data as $d) {
