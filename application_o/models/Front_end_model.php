...
class Front_end_model extends CI_Model {
    ...
    public function get_produc_with_price_and_details($id = '', $type='', $cat_id= ''){
        //$this->db->select('*');
        /* if($id != ''){
    ...
    public function get_coustomer($user_id){
        $data = $this->db->where('users_id', $users_id)->get('users')->result_array();
        return $data;
    }
    ...
    public function get_product_id($id = ''){   
        ...
        $detail = $this->db->select('id,name,images,brand,shor_desc,long_desc,other_desc,sku,category,video_id')->get('product')->result_array();
        return $detail;
    }
    public function get_produc_with_price($id = '', $cat_id= '', $type=''){
        ...
        if($cat_id != '' && $cat_id != '0'){
            $this->db->where(''.$cat_id.' IN (category)');
        }
        if($type != ''){
