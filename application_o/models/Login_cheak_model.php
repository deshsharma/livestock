...
class Login_cheak_model extends CI_Model {
    ...
    public function login_paravate($user, $password){
        ...
        $detail = $this->db->select('doctor_id, users_type, username,is_consultation_on, mobile, refral_code, ai_visiting_fee,company_partner, email, image, isactivated, is_payment, expertise_list, total_experience, rej_region, state,avaialable_for_visit,online_for_visit,visiting_fee, pincode, address_full, city,is_available_consultation, aadhar_no, adhaar_img,is_premium,vc_approve, CONCAT("'.IMAGE_PATH.'harpahu_merge_dev/uploads/doctore_doc/",vc_certificate) as vc_certificate')->where(''.$userdata.' = "'.$user.'" AND password ="'.$password.'"')->get('doctor')->result_array();
        if($detail){
            return $detail;
        }else{
            return false;
