...
function filter(inlineRadioOptions){
 ...
 function status(id , status){
  if (confirm('Are you sure you want to change the status')) {
    var person = '';
      ...
      start = 0;
  ajaxloader.load("https://www.livestoc.com/webservices_new_2/animals/get_animals_list'?id='"+id+"&status="+status+"&type="+$()+"&start="+start+"&perpage="+per_page+"&region="+person+"category_id="+category_id+"gender="+$("input:radio[name=inlineRadioOptions]:checked").val()+"agefrom="+min_age+"ageto="+max_age, function(resp){
 ...
 var num_display_entries,items_per_page,num_entries,start;
      ...
      function loadData(start,per_page,event_type){
                ...
                ajaxloader.load("https://www.livestoc.com/webservices_new_dev/animals/get_animals_list?is_pregnant=&gender="+$("input:radio[name=inlineRadioOptions]:checked").val()+"&category_id="+$("#animal_category").val()+"&lactation=&agefrom="+$("#min_age").val()+"&users_id=&listtype="+$('#type').val()+"&page=1&ageto="+$("#max_age").val()+"&isactivated=1", function(resp){
                              var data = resp;
