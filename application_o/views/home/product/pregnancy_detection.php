<input type="hidden" id="users_id" name="users_id" value="<?php echo $this->session->userdata('users_id') ? $this->session->userdata('users_id') : 0; ?>">
 <?php 
 //print_r($_REQUEST);
 $user_details = $this->api_model->get_data('users_id = "'.$this->session->userdata('users_id').'"', 'users as us', '', 'users_id,full_name,mobile,mobile_code,address,email,address_id,zone_id,(select address2 from address_mst where address_id = us.address_id) as address2,(select city from address_mst where address_id = us.address_id) as city,(select name from zone where zone_id = us.zone_id) as zone,(select district from address_mst where address_id = us.address_id) as district,(select postal_code from address_mst where address_id = us.address_id) as postal_code,(select latitude from address_mst where address_id = us.address_id) as latitude,(select longitude from address_mst where address_id = us.address_id) as longitude');
    ...
    <!-- <label type="submit" class="btn btn-primary"><?= $this->webLanguage['Sample Collection Center Application Form']?></label></br> -->
      <form action="" method="POST" id="userdetail" enctype="multipart/form-data"> 
        ...
        <div class="form-group">
          <label for="inputAddress"><?= $this->webLanguage['Form Name']?></label>
          <input type="text" value="" name = "business_name" class="form-control" id="inputBusiness" placeholder="Form Name">
        </div>
        <div class="form-group">
        ...
        <div class="form-row">
          <div class="form-group col-md-6">
            <label for="inputSample4"><strong>No Of Sample</strong></label>
            <input type="text" value="" name="sample" class="form-control" id="inputSample" required />
          </div>
        
          <div class="form-group col-md-6">
            <?php 
              $lab_det = $this->api_model->get_data('ispaid = "1"', 'lab_reg', 'distance', 'id, users_id, name,adress, district, state, city,location, latitude,langitude, phone, email, business_name, ispaid , IFNULL(( 3959 * acos( cos( radians(".$latitude.") ) * cos( radians( latitude ) ) * cos( radians( langitude ) - radians (".$langitude.") ) + sin( radians(".$latitude.") ) * sin( radians( latitude ) ) ) ),0) AS distance');
              //print_r($lab_det);
            ?>
            <label for="inputSample4"><strong>Select Lab</strong></label>
            <!-- <input type="text" value="" name="sample" class="form-control" id="inputSample"> -->
            <select name="lab_id" class="form-control" id="cars" required>
              <option value="">Select Lab</option>
              <?php
                foreach($lab_det as $ld){?>
                  <option value="<?= $ld['id']?>"> <?= $ld['name']?>,(Phone Number - <?=$ld['phone']?>),(<?= $english_format_number = number_format($ld['distance'], 2, '.', '')?>KM)</option>
               <?php }?>
