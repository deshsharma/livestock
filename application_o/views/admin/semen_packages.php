	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              ...
		              <a class="white" href="<?= base_url('admin/semen_package_add'); ?>"><div class="btn btn-info but_set">
		                 Add Package
		              </div></a>
...
<script type="text/javascript">
 ...
 function status(id , status){
    if (confirm('Are you sure you want to change the status')) {
 	ajaxloader.load("<?php echo base_url('admin/semen_package_status').'?id=' ?>"+id+"&status="+status, function(resp){
			                      	var data = resp;
			                     	...
			                     	if(str['error']=='1'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            if(item.is_active != '1'){
												...
												status += "<a href='<?= base_url('admin/semen_packages_edit/')?>"+item.id+"'><button type='button' class='btn btn-primary btn-flat'>Edit</button></a>";
                                            }else{
			                            	...
			                            	//status += "<a href='superv_edit/"+item.id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
			                            	 tr += "<tr>\
                                                           ...
                                                           <td>"+item.per_animal_price+"</td>\
                                                           <td>"+item.animal_quantity+"</td>\
			                                               <td>"+status+"</td>\
			                                               </tr>";
			                            });
			                       	}
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/semen_packages_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str['error']=='1'){ 
