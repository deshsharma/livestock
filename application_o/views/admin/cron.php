	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">CRON MANAGEMENT</h3>
		              <a class="white" href="<?= base_url('admin/add_cron'); ?>"><div class="btn btn-info but_set">
		                 Add Cron
		              </div></a>
...
<script type="text/javascript">
 ...
 function status(id , status){
    if (confirm('Are you sure you want to change the status')) {
 	ajaxloader.load("<?php echo base_url('admin/cron_status').'?id=' ?>"+id+"&status="+status, function(resp){
			                      	var data = resp;
			                     	...
			                     	if(str['error']=='1'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            if(item.isactivated != '1'){
												...
												status += "<a href='<?= base_url('admin/edit_cron/')?>"+item.id+"'><button type='button' class='btn btn-primary btn-flat'>Edit Cron</button></a>";
                                            }else{
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/cron_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str['error']=='1'){ 
