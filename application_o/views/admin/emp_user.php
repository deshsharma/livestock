          ...
          <div class="col-xs-12">
            <div class="box">
              <div class="box-header with-border">
                  <h3 class="box-title">COUSTOMER LIST</h3>
                  <div class="">
							<span><bold>Total User : <span id="tot_count" style="color: #f30808;"></span></bold></span>
				</div>
...
<script type="text/javascript">
 ...
 function status(id , status){
  ...
  ajaxloader.load("<?php echo base_url('admin/gvd_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person, function(resp){
                              var data = resp;
                      ...
                      var status = '';
                      if(item.is_active != '1'){
                                                status += "<button type='button' onclick='status("+item.gvd_id+", 1)' class='btn btn-danger btn-flat'>Active</button>"
                                                //status += "<button type='button' onclick='status("+item.doctor_id+", 2)' class='btn btn-primary btn-flat'>Reject</button>";
                                            }else{
                                                status += "<button type='button' onclick='status("+item.gvd_id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
                                                //status += "<button type='button' onclick='status("+item.doctor_id+", 2)' class='btn btn-primary btn-flat'>Reject</button>";
                                            }
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
                ajaxloader.load("<?php echo base_url('employee/employee_user_search').'?name=' ?>"+$('#product_name').val()+"&id=<?= $id ?>&start="+start+"&perpage="+per_page, function(resp){
                              var data = resp;
                              var str =JSON.parse(data);
                              var tr = '';
                            if(str.count[0].count=='0'){ 
                              $('#tot_count').text(str.count[0].count);
                              tr += "<tr><td colspan='7' align='center'>No record found!</td></tr>";
                              }
                              else{
                                var result = str;
                                var type = '';
                                $('#tot_count').text(result.count[0].count);
                                $('#count').val(result.count[0].count);
                                delete result.count ;
                                  $.each(result, function(index, item){ 
                                    var status = '';
                                    // if(item.users_type == 'pvt_field'){
                                    //     type = 'Field Executive';
                                    // }else if(item.users_type == 'pvt_milk'){
                                    //     type = 'Yield Checker';
                                    // }else if(item.users_type == 'pvt_diss'){
                                    //   type = 'Assistant Dirstrict Manager'
                                    // }
                                    if(item.premium == '0'){
                                      type = 'Non Premium';
                                    }else{
                      ...
                      // }
                                    ...
                                    //  status += "<a href='<?= base_url('admin/sub_dis_list/')?>"+item.doctor_id+"'><button type='button' class='btn btn-primary btn-flat'>List</button></a>";
                                     tr += "<tr>\
