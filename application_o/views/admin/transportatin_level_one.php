...
<?php //print_r($_SESSION); ?>
				  ...
				  <?php //if($_SESSION['type'] == '5' || $_SESSION['type'] == '1' || $_SESSION['type'] == '11'){?>
		              <a class="white" href="<?= base_url('admin/transportation_level/1'); ?>"><div class="btn btn-info but_set">
					  		Add Transportatin Level One
		              </div></a>
				  <?php //} ?>
 ...
 $(document).ready(function() {
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     if($_SESSION['status'] == 1){ ?>
        var get_url = "<?php echo base_url('api/listing_authority').'?user_type=25&admin_id=66'?>&search="+$('#product_name').val();
     <?php }else{ 
					if($_SESSION['type'] == '10' || $_SESSION['type'] == '11'){
								$admin_data = $this->api_model->get_seman_company_id($_SESSION['user_id']);  ?>
								var get_url = "<?php echo base_url('api/listing_authority').'?user_type=25&admin_id='.$admin_data[0]['super_admin_id'].''; ?>&search="+$('#product_name').val();
						<?php }else{ ?>
								var get_url = "<?php echo base_url('api/listing_authority').'?user_type=25&admin_id='.$_SESSION['user_id'].''; ?>&search="+$('#product_name').val();
						<?php } 
