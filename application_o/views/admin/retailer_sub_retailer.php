	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">RETAILER MANAGEMENT</h3>
					  <div class="">
							<span><bold>Total Retailer &nbsp;:&nbsp; <span id="tot_dis" style="color: #f30808;"></span></bold></span>
					   </div>
...
<script type="text/javascript">
 ...
 function status(id , status){
    if (confirm('Are you sure you want to change the status')) {
 	ajaxloader.load("<?php echo base_url('admin/retailer_sub_retailer_status').'?id=' ?>"+id+"&admin_id="+$('#user_id').val()+"&status="+status, function(resp){
			                      	var data = resp;
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/retailer_sub_retailer_search').'?name=' ?>"+$('#product_name').val()+"&admin_id="+$('#user_id').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
