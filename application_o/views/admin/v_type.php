	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              ...
		              <a class="white" href="<?= base_url('admin/v_type_add'); ?>"><div class="btn btn-info but_set">
		                 Add Vehicle Type
		              </div></a>
...
<script type="text/javascript">
 ...
 function status(id , status){
    if (confirm('Are you sure you want to change the status')) {
 	ajaxloader.load("<?php echo base_url('admin/v_type_status').'?id=' ?>"+id+"&status="+status, function(resp){
			                      	var data = resp;
			                     	...
			                     	if(str['error']=='1'){ 
			                            ...
			                            $.each(result, function(index, item){
			                            	...
			                            	status += "<a href='<?= base_url('admin/edit_v_type/')?>"+item.id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
			                            	 tr += "<tr>\
			                                               <td>"+item.id+"</td>\
			                                               <td>"+item.vehicals_name+"</td>\
			                                               <td>"+status+"</td>\
			                                               </tr>";
			                            }); 
			                       	}
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/v_type_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str['error']=='1'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            if(item.is_activated != '1'){
                                                status += "<button type='button' onclick='status("+item.id+", 1)' class='btn btn-danger btn-flat'>Active</button>"
												//status += "<a href='<?= base_url('admin/edit_v_type/')?>"+item.id+"'><button type='button' class='btn btn-primary btn-flat'>Account</button></a>";
                                            }else{
                                                status += "<button type='button' onclick='status("+item.id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
												//status += "<a href='<?= base_url('admin/edit_v_type/')?>"+item.id+"'><button type='button' class='btn btn-primary btn-flat'>Account</button></a>";
                                            }
