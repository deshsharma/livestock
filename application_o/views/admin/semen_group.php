	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              ...
		              <a class="white" href="<?= base_url('admin/semen_group_add'); ?>"><div class="btn btn-info but_set">
		                 Add Semen Group
		              </div></a>
...
<script type="text/javascript">
 ...
 function status(id , status){
    if (confirm('Are you sure you want to change the status')) {
 	ajaxloader.load("<?php echo base_url('admin/semen_account_status').'?id=' ?>"+id+"&status="+status, function(resp){
			                      	var data = resp;
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/semen_account_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str['error']=='1'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            // }else{
                                                // status += "<button type='button' onclick='status("+item.users_id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
												status += "<a href='<?= base_url('admin/semen_group_edit/')?>"+item.id+"/'><button type='button' class='btn btn-primary btn-flat'>Edit</button></a>";
