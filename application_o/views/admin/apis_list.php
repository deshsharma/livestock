	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">APIS DETAILS LIST</h3>
		              <div class="pull-right Search_set1"> 
                      <input type="text" class="glyphicon-search" placeholder="Method" id="product_name" name="product_name">
                      <span class="glyphicon glyphicon-search" onclick="filterData();"></span> 
                      <input type="hidden" value="" id="count" >
                  </div>  
                <a class="white" href="<?= base_url('admin/add_apis'); ?>"><div class="btn btn-info but_set">
                     Add API
                  </div></a>
	            </div>
		         <div class="box-body">
		         	<table id="tabdata" class="table table-bordered table-striped" style=" table-layout: fixed; width:100%">
		                <thead>
			                <tr>
                        <th style="width: 3%;">ID</th>
                        <th style="width: 80%;">Link url</th>
                        <th></th>
			                </tr>
		                </thead>
		                <tbody>
...
<script type="text/javascript">
 ...
 function del(id)
 {
 	  if (confirm('Are you sure you want to Delete')) {
 	    ajaxloader.load("<?php echo base_url('admin/apis_del').'?id=' ?>"+id, function(resp){
          var data = resp;
          var str =JSON.parse(data);
          var tr = '';
         	if(str['error']=='1'){ 
                ...
                $.each(result, function(index, item){
					      var status = '';
      					if(item.api_id != 1){
                  status = "<button type='button' onclick='del("+item.api_id+")' class='btn btn-danger btn-flat'>Delete</button>";
      					  }										 
                	status += "<a href='superv_edit/"+item.api_id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
                	  tr += "<tr>\
                       <td>"+item.api_id+"</td>\
                       <td>"+item.method+"</td>\
                       <td>"+item.linkurl+"</td>\
 ...
 $(document).ready(function() {
    ...
    function loadData(start,per_page,event_type){
			ajaxloader.load("<?php echo base_url('admin/apis_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
            var data = resp;
            var str =JSON.parse(data);
            var tr = '';
              ...
              $.each(result, function(index, item){
                ...
                var status  = "<button type='button'  class='btn btn-danger btn-flat'>Delete</button>";

                var status = status+"<a href='apis_edit/"+item.api_id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";

            	  tr += "<tr>\
                   <td>"+item.api_id+"</td>\
                   <td style='overflow: scroll;'>"+item.linkurl+"</td>\
                   <td align='center'>"+status+"</td>\
                   </tr>";
                });
            }
