	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">ANIMAL EVALUATION</h3>
		              <!-- <a class="white" href="<?= base_url('admin_composition/composition_add'); ?>"><div class="btn btn-info but_set">
		                 Add Composition
		              </div></a> -->
...
<script type="text/javascript">
 ...
 function status(id , status){
	if (confirm('Are you sure you want to change the status')) {
 	...
 	ajaxloader.load("<?php echo base_url('admin_composition/composition_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person+"&name="+$('#product_name').val(), function(resp){
			                      	var data = resp;
                                  ...
                                  delete result.count ;
			                            $.each(result, function(index, item){
			                            	...
			                            	status += "<a href='<?= base_url() ?>admin_composition/edit/"+item.id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
			                            	 tr += "<tr>\
			                                               <td>"+item.name+"</td>\
                                                           <td>"+item.unit+"</td>\
			                                               <td>"+status+"</td>\
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('evaluation/eve_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
                                	...
                                	delete result.count ;
			                            $.each(result, function(index, item){
                                            ...
                                            //     status += "<button type='button' onclick='status("+item.id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
                                            // }
			                            	status += "<a href='<?= base_url() ?>evaluation/edit/"+item.id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
											status += "<a href='<?= base_url() ?>evaluation/animal/"+item.animal_id+"'><button type='button' class='btn btn-danger btn-flat'>View</button></a>";
			                            	 tr += "<tr>\
                                                           <td>#"+item.animal_id+"</td>\
                                                           <td>"+item.user_name+'('+item.user_mobile+')'+"</td>\
			                                               <td>"+item.admin_name+'('+item.admin_mobile+')'+"</td>\
														   <td>"+item.evaluation_price+"</td>\
														   <td>"+status+"</td>\
			                                               </tr>";
			                            }); 
			                       	}
