	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">Repeat Breading Orders</h3>
		              <!-- <a class="white" href="<?= base_url('admin/unit_add'); ?>"><div class="btn btn-info but_set">
		                 Add Unit
		              </div></a> -->
		         	...
		         	<table id="tabdata" class="table table-bordered table-striped">
		                <thead>
			                <tr>
                              ...
                              <th>VT Name(Mobile)</th>
                              <th>Doctor Name(Mobile)</th>
			                  <th>Date</th>
                              <th></th>
			                </tr>
		                </thead>
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('repeat_breading/search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
