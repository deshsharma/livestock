	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">Delivery Partner</h3>
		              <div class="pull-right Search_set1"> 
					  		<a href="<?= base_url() ?>admin/offline_delivery_partner_transaction"><button class="btn btn-success">All Offline Transaction</button></a>
...
<script type="text/javascript">
 ...
 function status(id , status){
	if (confirm('Are you sure you want to change the status')) {
 	...
 	ajaxloader.load("<?php echo base_url('admin/delivery_partner_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person, function(resp){
			                      	var data = resp;
			                     	...
			                     	if(str.count[0].count=='0'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            if(item.isactivated != '1'){
												...
												status += "<a href='<?= base_url('admin/delivery_partner_account') ?>/"+item.admin_id+"'><button type='button' class='btn btn-primary btn-flat'>Account</button></a>";
											}else{
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/delivery_partner_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            if(item.isactivated != '1'){
                                                status += "<button type='button' onclick='status("+item.admin_id+", 1)' class='btn btn-danger btn-flat'>Active</button>"
												status += "<a href='<?= base_url('admin/delivery_partner_account') ?>/"+item.admin_id+"'><button type='button' class='btn btn-primary btn-flat'>Order Account</button></a>";
												status += "<a href='<?= base_url('admin/delivery_partner_cod') ?>/"+item.admin_id+"'><button type='button' class='btn btn-success btn-flat'>COD Account</button></a>";
												status += "<a href='<?= base_url('admin/delivery_partner_insentive') ?>/"+item.admin_id+"'><button type='button' class='btn btn-success btn-flat'>COD Account</button></a>";
											}else{
                                                status += "<button type='button' onclick='status("+item.admin_id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
                                                status += "<a href='<?= base_url('admin/delivery_partner_account') ?>/"+item.admin_id+"'><button type='button' class='btn btn-primary btn-flat'>Order Account</button></a>";
												status += "<a href='<?= base_url('admin/delivery_partner_cod') ?>/"+item.admin_id+"'><button type='button' class='btn btn-success btn-flat'>COD Account</button></a>";
												status += "<a href='<?= base_url('admin/delivery_partner_insentive') ?>/"+item.admin_id+"'><button type='button' class='btn btn-primary btn-flat'>Incentive Account</button></a>";
											}
