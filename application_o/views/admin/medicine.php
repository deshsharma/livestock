	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              ...
		              <a class="white" href="<?= base_url('admin/add_medicine'); ?>"><div class="btn btn-info but_set">
		                 Add Medicine
		              </div></a>
...
<script type="text/javascript">
 ...
 function status(id , status){
    if (confirm('Are you sure you want to change the status')) {
 	ajaxloader.load("<?php echo base_url('admin/medicine_status').'?id=' ?>"+id+"&status="+status, function(resp){
			                      	var data = resp;
                                    ...
                                    if(str.count[0].count=='0'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            if(item.isactive != '1'){
												...
												status += "<a href='<?= base_url('admin/edit_medicine/')?>"+item.id+"'><button type='button' class='btn btn-primary btn-flat'>Edit Medicine</button></a>";
                                            }else{
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/medicine_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
