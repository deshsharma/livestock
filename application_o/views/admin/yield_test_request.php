	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">YIELD TEST REQUEST</h3>
 ...
 $(document).ready(function() {
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/yield_test_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page+"&user_id="+user_id, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
                                ...
                                delete result.count ;
			                            $.each(result, function(index, item){
			                            	...
			                            	if(item.request_status == '2'){
			                            		...
			                            		type = 'Payment From Wallet';
			                            	}
			                            	
			                            	var opt = "";
			                            	opt = "<a href='<?= base_url('admin/yield_test_view/')?>"+item.log_id+"' class='btn btn-primary'>view</a>";
			                            	 tr += "<tr>\
                                                           <td>"+item.log_id+"</td>\
