	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">SEMEN COMPANY GROUP PRICE MANAGEMENT</h3>
		              <a class="white" href="<?= base_url('admin/semen_company_price_add'); ?>"><div class="btn btn-info but_set">
		                 Add Semen Company Group Price
		              </div></a>
		              <div class="pull-right Search_set1"> 
						   <input type="text" class="glyphicon-search" placeholder="Company Name" id="product_name" name="product_name">
		         	...
		         	<table id="tabdata" class="table table-bordered table-striped">
		                <thead>
			                <tr>
			                  ...
			                  <th>Price</th>
                              <th>Offer Price</th>
                              <th>Advance Booking Price</th>
                              <th>AI Service Charges</th>
                              <th>Company Charges</th>
                              <th>Company offer Charges</th>
			                  <th></th>
			                </tr>
		                </thead>
		                <tbody>
...
<script type="text/javascript">
 ...
 function status(id , status){
    if (confirm('Are you sure you want to change the status')) {
 	ajaxloader.load("<?php echo base_url('admin/semen_company_price_status').'?id=' ?>"+id+"&status="+status, function(resp){
			                      	var data = resp;
			                     	...
			                     	if(str['error']=='1'){ 
			                            ...
			                            $.each(result, function(index, item){
											...
											// 	status += "<a href='<?= base_url('admin/semen_group_edit/')?>"+item.users_id+"/1'><button type='button' class='btn btn-primary btn-flat'>Account</button></a>";
                                            // }else{
                                                // status += "<button type='button' onclick='status("+item.users_id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
												status += "<a href='<?= base_url('admin/semen_company_price_edit/')?>"+item.id+"/'><button type='button' class='btn btn-primary btn-flat'>Edit</button></a>";
                                            ...
                                            // }
                                            if(item.isactive != 1){
                                                status += "<button type='button' onclick='status("+item.id+", 1)' class='btn btn-success btn-flat'>Active</button>";
                                            }else{
                                                status += "<button type='button' onclick='status("+item.id+", 0)' class='btn btn-danger btn-flat'>Deactive</button>";
                                            }									 
			                            	//status += "<a href='superv_edit/"+item.id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
			                            	 tr += "<tr>\
			                                               <td>"+item.group_name+"</td>\
                                                           <td>"+item.company_name+"</td>\
                                                           <td>"+item.farmer_price+"</td>\
                                                           <td>"+item.farmer_offer_price+"</td>\
                                                           <td>"+item.advance_booking_price+"</td>\
                                                           <td>"+item.ai_service_price+"</td>\
			                                               <td>"+item.company_charges+"</td>\
                                                           <td>"+item.company_offer_charges+"</td>\
			                                               <td>"+status+"</td>\
			                                               </tr>";
			                            });
			                       	}
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/semen_company_price_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str['error']=='1'){ 
