	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">Semen Companies</h3>
		         	...
		         	<table id="tabdata" class="table table-bordered table-striped">
		                <thead>
			                <tr>
			                  <th>ID</th>
			                  <th>Log ID</th>
                              <th>Semen Stock ID</th>
                              <th>Semen Stock Price</th>
                              <th>Semen Stock Qty</th>
                              <th>Total Paid</th>
			                  <th></th>
			                </tr>
		                </thead>
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/get_semen_transaction').'?name=' ?>"+$('#product_name').val()+"&id="+$('#id').val()+"&start="+start+"&perpage="+per_page+"&type=18", function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
			                            ...
			                            $.each(result, function(index, item){
                                            ...
                                            //     status += "<button type='button' onclick='status("+item.admin_id+", 1)' class='btn btn-danger btn-flat'>Active</button>"
                                            // }else{
                                                status += "<a href='<?= base_url() ?>admin/semen_transaction/"+item.admin_id+"'<button type='button' class='btn btn-success btn-flat'>Check Transaction</button></a>";
                                            //}
			                            	...
			                            	//status += "<a href='superv_edit/"+item.id+"'><button type='button' class='btn btn-info btn-flat'>Edit</button></a>";
			                            	 tr += "<tr>\
			                                               <td>"+item.id+"</td>\
			                                               <td>"+item.log_id+"</td>\
                                                           <td>"+item.semen_stock_id+"</td>\
                                                           <td>"+item.semen_stock_price+"</td>\
                                                           <td>"+item.semen_stock_qty+"</td>\
