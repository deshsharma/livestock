...
<section class="content">
                  ...
                  <?php if($_SESSION['status'] == '18' || $_SESSION['status'] == '1'){ ?>
 ...
 function livestoc(id , status){
	if (confirm('Are you sure you want to change the Livestoc Product status')) {
		var person = '';
			if(status == '2'){
			...
			start = 0;
 	ajaxloader.load("<?php echo base_url('admin/product_livetoc_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person, function(resp){
                              ...
                              tr += "<div class='col-md-12' align='center'>No record found!</div>";
                                ...
                                delete result.count ;
			                            $.each(result, function(index, item){
											...
											// }                   
                                            if(item.livestoc_status == '0'){
                                              status += "<button type='button' onclick='livestoc("+item.id+", 1)' class='btn btn-sm btn-info'>Livestoc Product</button>";
                                            }else{
                                              status += "<button type='button' onclick='livestoc("+item.id+", 0)' class='btn btn-sm btn-info'>Non Livestoc Product</button>";
                                            }
                                            if(item.isactive != '1'){
                                             ...
                                             <?php if($_SESSION['status'] == '18' || $_SESSION['status'] == '1'){ ?>
                                              ...
                                              status += "<a href='<?= base_url('admin/product_view') ?>/"+item.id+"'><button type='button' onclick='' class='btn btn-sm btn-info'>View</button></a>";
																								//status += "<a href='language_library_edit/"+item.id+"'><button type='button' onclick='status("+item.doctor_id+", 2)' class='btn btn-primary btn-flat'>Edit</button></a>";
																						}else{
                                              //status += "<a href='<?= base_url('admin/product_lead') ?>/"+item.id+"'><button type='button' onclick='' class='btn btn-sm btn-info'>Lead</button></a>";
                                              <?php if($_SESSION['status'] == '18' || $_SESSION['status'] == '1'){ ?>
                                            ...
                                            status += "<a href='product_edit/"+item.id+"'><button type='button' class='btn btn-sm btn-danger'>Edit</button></a>";
