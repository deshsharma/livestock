	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
		              <h3 class="box-title">USER NEGOTIATION</h3>
		              ...
		              <div class="pull-right Search_set1"> 
						   <input type="text" class="glyphicon-search" placeholder="User Name" id="product_name" name="Animal ID">
...
function cart_request(animal_id, user_id, type, qty, evaluation_price, livestoc_price, bidding_price){
	$.ajax({
		url: "https://www.livestoc.com/harpahu_merge_dev/animal_ecomm/add_cart?animal_id="+animal_id+"&user_id="+user_id+"&type="+type+"&qty="+qty+"&evaluation_price="+evaluation_price+"&livestoc_price="+livestoc_price+"&bidding_price="+bidding_price,
		cache: false,
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('user_negotiation/search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page, function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
                                ...
                                delete result.count ;
			                            $.each(result, function(index, item){
											...
											// }
                                               // status += "<button type='button' onclick='status("+item.id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
											status += "<button type='button' onclick='cart_request("+item.product_id+", "+item.users_id+", 1, 1, "+item.evaluation_price+", "+item.livestoc_price+", "+item.bidding_price+")' class='btn btn-success btn-flat'>Add To Cart</button>";										 
			                            	status += "<a href='coupans/create/"+item.id+"/"+item.product_id+"/"+item.users_id+"/animals' target='_blank'><button type='button' class='btn btn-info btn-flat'>Create Coupans</button></a>";
			                            	 tr += "<tr>\
