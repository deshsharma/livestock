  ...
  <body>
    ...
    <main>
      ...
      <table border="0" cellspacing="0" cellpadding="0">
        ...
        $lenth = sizeof($stock);
        foreach($stock as $st){
          $stock_detail = $this->api_model->get_data('package_id = '.$st.'', 'package_masters');
          $semen = $this->api_model->get_data(' FIND_IN_SET(vaccination_id, "'.$stock_detail[0]['vacc_id'].'")','vaccination');
         //print_r($semen);
          if(!empty($log_data)){
            //echo "this is test";
            // exit;
                ...
                <tr>
                          <!-- <td class="no"><?= $i+1 ?></td> -->
                          <td class="desc"><h3><?= $stock_detail[0]['package_name']?></h3>
                          <?php foreach($semen as $se){
                              echo '<h5>'.$se['name'].'</br></h5>';
                          } ?>
                          </td>
    ...
    <footer>
      Invoice is created by computer.
    </footer>
  </body>
</html>