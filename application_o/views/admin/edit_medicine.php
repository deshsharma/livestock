...
<div class="abc" ><h3>ADD MEDICINE TYPE</h3></div>
...
<div class="abc_1" >
    ...
    <?php } ?>
        <?php echo form_open("admin/edit_medicine/".$data[0]['id'], ['onsubmit'=>'return myFunction()']);?>
            ...
            <div class="col-md-3" > 
                ...
                <?php
                $cat = $this->api_model->get_data('','medicine_type'); 
                    foreach($cat as $ca){
                ?>
                    <option value="<?= $ca['id'] ?>" <?php if($data[0]['type'] == $ca['id']){ echo "Selected"; } ?>><?= $ca['name'] ?></option>
                <?php } ?>
                </select>
            ...
            <div class="col-md-3" > 
                ...
                <?php $cat = $this->api_model->get_data('','unit'); 
                    foreach($cat as $ca){
                ?>
                    <option value="<?= $ca['id'] ?>" <?php if($data[0]['unit'] == $ca['id']){ echo "Selected"; } ?>><?= $ca['name'] ?></option>
                <?php } ?>
                </select>
                    <?php// echo form_input(['type'=>'text','name'=>'category', 'value'=>'', 'id'=>'category','class'=>'ch_manset padd_set']) ?>
        	</div>
            ...
            <div class="col-md-3" > 
                ...
                <?php $cat = $this->api_model->get_data('','category'); 
                    foreach($cat as $ca){
                ?>
                    <option value="<?= $ca['category_id'] ?>" <?php if(in_array($ca['category_id'], $category)){ echo "selected"; } ?>><?= $ca['category'] ?></option>
                <?php } ?>
                </select>
                    <?php// echo form_input(['type'=>'text','name'=>'category', 'value'=>'', 'id'=>'category','class'=>'ch_manset padd_set']) ?>
        	</div>
            ...
            <div class="col-md-3" > 
                ...
                <div class="col-md-9">
                    <?php echo form_input(['type'=>'text','name'=>'composition', 'value'=>$data[0]['composition'], 'id'=>'composition','class'=>'ch_manset padd_set']) ?>
        	</div>
