	        ...
	        <div class="col-xs-12">
	          <div class="box">
	          	<div class="box-header with-border">
					  ...
					  <input type="" name="transaction_id" id="transaction_id" class="glyphicon-search" value="" placeholder="Transaction ID">
...
<script type="text/javascript">
 ...
 $('#pay').click(function(){
	...
	//alert(selected);
	ajaxloader.load("<?php echo base_url('admin/order_insentive_pay').'?id=' ?>"+selected+"&delivery_id=<?= $id ?>&transaction_id="+$('#transaction_id').val(), function(resp){
		if(resp == true){
 			...
 			ajaxloader.load("<?php echo base_url('admin/order_status').'?id=' ?>"+id+"&status="+status+"&start="+start+"&perpage="+per_page+"&region="+person+"&delivery_id=<?= $id ?>", function(resp){
			                     	...
			                     	if(str.count[0].count=='0'){ 
			                     		tr += "<tr><td colspan='8' align='center'>No record found!</td></tr>";
																...
																$('.total').text(result.count[0].total_insentive);
                                delete result.count ;
                      ...
                      //                           status += "<button type='button' onclick='status("+item.id+", 0)' class='btn btn-success btn-flat'>Deactive</button>";
											...
											var payment_state = '';
											if(item.delivery_partner_insentive_payment_status == '0'){
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function initPagination() {
      ...
      function loadData(start,per_page,event_type){
      					ajaxloader.load("<?php echo base_url('admin/delivery_partner_order_search').'?name=' ?>"+$('#product_name').val()+"&start="+start+"&perpage="+per_page+"&delivery_id=<?= $id ?>&cod=2", function(resp){
			                      	var data = resp;
			                      	var str =JSON.parse(data);
			                      	var tr = '';
			                     	if(str.count[0].count=='0'){ 
